...
@Component({
  selector: 'app-fdp-lou-p5',
  templateUrl: './fdp-lou-p5.component.html',
  styleUrls: ['./fdp-lou-p5.component.scss']
})
export class FdpLouP5Component implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.music = new Audio('/assets/images/loup/jeux/5/a.mp3');
  ...
  public verifyAnswer() {
    if(this.answer.value.toUpperCase() === 'CHAT') {
      this.music.pause();
      this.win.play();
      setTimeout(() => {
        this.router.navigateByUrl('LouPD/yurkrtxc');
      }, 2000);
    } else {
      this.loose.play();
