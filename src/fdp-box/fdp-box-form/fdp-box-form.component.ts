...
export class FdpBoxFormComponent {
  ...
  constructor(private fb: FormBuilder, private fdpBoxService: FdpBoxService) {
    this.box = new FdpBox();
    this.error = "";
    this.createForms();
  }
  ...
  createForms() {
    this.filesForm = new FormGroup({
      box_sound: new FormControl(this.box.sound.id, [Validators.required]),
      box_miniature: new FormControl(this.box.miniature.id, [
        Validators.required
      ])
    });
    this.informationsForm = new FormGroup({
      box_title: new FormControl(this.box.title, [Validators.required]),
      box_description: new FormControl(this.box.description, [])
    });
  }

  sendBox() {
    this.fdpBoxService.createBox(this.box).subscribe(
      res => {
        if (res.box) {
