import {PlaylistYoutubeType} from '../../_generated/graphql.schema';

export const YOUTUBE_PLAYLISTS: PlaylistYoutubeType[] = [
  {
    title: 'AAAA',
    youtubeId: 'PL6EUPPQAaktUp_pFVtDF2M_WVV9Hf143P',
    year: '2014 - 2015',
  },
  {
    title: 'BBBB',
    youtubeId: 'PL6EUPPQAaktUT2nlY2qfb1K_Ttz5MYKuN',
    year: '2015 - 2016',
  },
  {
    title: 'CCCC',
    youtubeId: 'PL6EUPPQAaktVJDGcl2aRS9KeTa_H1f-_c',
    year: '2016 - 2017',
  },
  {
    title: 'DDDD',
    youtubeId: 'PL6EUPPQAaktVKuIob29iz5o9zNtVb6vYe',
    year: '2017 - 2018',
  },
  {
    title: 'EEEE',
    youtubeId: 'PL6EUPPQAaktWSqtssejVQbJeaVVIecm1Q',
    year: '2018 - 2019',
  },
  {
    title: 'FFFF',
    youtubeId: 'PL6EUPPQAaktXmUWBJ696DtYZO21wuBpCU',
    year: '2019 - 2020',
  }
];
