...
export class SoundWaveComponent implements OnInit {
  ...
  constructor(
    ...
    this.slice = new EventEmitter<ISoundSlice>();
  }
  ...
  public loadFile() {
    this.http.get(this.resource.url, {responseType: 'blob'})
      .subscribe(async (blob: Blob) => {
        this.initWaveViewers();
        const arrayBuffer = await new Response(blob).arrayBuffer();
        from(this.soundService.audioContext.decodeAudioData(arrayBuffer))
  ...
  private getNearestSlicer(audioValue: number) {
    return Math.abs(audioValue - this.audioSliceFrom) < Math.abs(audioValue - this.audioSliceTo) ? this.FROM : this.TO;
  }
  ...
  private initWaveViewers() {
    ...
    this.waveSlice.nativeElement.setAttribute('fill', '#fff');
    ...
    this.waveReaded.nativeElement.setAttribute('fill', '#fff');
  }
